OPL developer package v1.56 for Nokia 9300/9500 Communicator Series.
...
Note: the target files should be installed on a Nokia 9300/9500 Series Communicator. 

PC/SDK Installation
-------------------
1) Ensure you have a S80 C++ SDK (available from http://www.forum.nokia.com/) installed on your PC.
2) From this C++ SDK installation, delete the following folder and its contents (if present):
...
5) Although the OPXs are installed from the \epoc32\ tree in step 4 above, SIS files for the WINS UDEB variant of 
...
\Binaries\SisFiles\PC\OPX\udeb\ into the \epoc32\release\wins\udeb\ folder of your SDK installation.

This will ensure the WINS OPX SIS files are in the recommended location on your SDK. You can then code your 
PKG file to look in <EPOCROOT>\epoc32\release\wins\udeb\ for the OPX SIS files you need to include in your 
own WINS distribution.

6) The DemoOPL application can also be installed under WINS. To do this copy the SIS file 
from \Binaries\SisFiles\PC\Other\Udeb\ onto the emulated C:\ drive of the emulator (usually 
<EPOCROOT>\epoc32\wins\c\). You can then install DemoOPL the next time you run WINS.
...
8) Create a \epoc32\release\thumb\urel\ folder on your SDK installation. In to this copy the files in 
...
NOTE: If you are ONLY using the SDK installation for OPL development then you can remove some redundant 
...
Series Communicator to your PC and install \Binaries\SisFiles\Target\Other\OPL_S80.sis in the normal way as described in the 
manual. If you wish to verify your installation you can optionally install 
\Binaries\SisFiles\Target\Other\DemoOPL_S80.sis which will allow you to test OPL is properly installed.


To develop on the Nokia 9300/9500 Series Communicator itself, additionally:

1) Install \Binaries\SisFiles\Target\Other\OPLDev_S80.sis
